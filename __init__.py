...
The DB way is currently very basic. All schema modules must import schema_orm,
this module holds basic SQLAlchemy global variables, such as the Base for all
tables (which needs to be the same one for everyone), or the default session
created by importing any of this modules. If you need to import all schemas at
once, just import schema_mambu, which should hold them all. If, on the other
hand, you want to use a specific schema module, just be careful when
dependencies happen, for example if you use schema_groups to get groups,
a group can have loans, but, to get them you need to import schema_loans too.
