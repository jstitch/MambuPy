...
class MambuRepaymentTests(unittest.TestCase):
    ...
    def test___repr__(self):
        ...
        with mock.patch.object(mamburepayment.MambuStruct, "__init__", build_mock_rep_1):
            rep = mamburepayment.MambuRepayment(urlfunc=getrepaymentsurl, entid="mockrepayment")
            self.assertRegexpMatches(repr(rep), r"^MambuRepayment - duedate: {}".format(date.today().strftime("%Y-%m-%d")))
...
class MambuRepaymentsTests(unittest.TestCase):
    ...
    def test_convertDict2Attrs(self):
        ...
        with self.assertRaisesRegexp(AttributeError,"'MambuRepayments' object has no attribute 'mamburepaymentclass'"):
