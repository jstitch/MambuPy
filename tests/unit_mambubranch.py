    ...
    def test_setUsers(self):
        ...
        class my_users(object):
            def __init__(self, ids, usernames, userstates):
                self.attrs = [{'id':id, 'username':username, 'userState':userstate}
                              for id,username,userstate in zip(ids,usernames,userstates)]
            def __getitem__(self,item):
                return self.attrs[item]
        with mock.patch.object(mambubranch.MambuStruct, "connect", mock_connect),\
             mock.patch('MambuPy.rest.mambuuser.MambuUsers') as mock_mambuusers:
            my_users_instances = my_users(ids        = ["dummyUserId1","dummyUserId2","dummyUserId3"],
...
class MambuBranchesTests(unittest.TestCase):
    ...
    def test_convertDict2Attrs(self):
        ...
        with self.assertRaisesRegexp(AttributeError,"'MambuBranches' object has no attribute 'mambubranchclass'"):
